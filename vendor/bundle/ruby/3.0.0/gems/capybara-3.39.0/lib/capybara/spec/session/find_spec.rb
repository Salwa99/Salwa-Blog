...
Capybara::SpecHelper.spec '#find' do
  ...
  context 'with custom selector with custom filter' do
    ...
    it 'should not raise an ElementNotFound error from in a filter' do
      expect {
        @session.find(:beatle, 'John', fail: 'something')
      }.to raise_error(Capybara::ElementNotFound, /beatle "John"/)
    end
  end
  ...
  context 'with alternate filter set' do
    ...
    it 'should allow use of filters from custom filter set' do
      ...
      expect {
        @session.find(:id, 'test_field', filter_set: :value, with: 'not_monkey')
    ...
    it 'should allow use of filter set from a different selector' do
      ...
      expect {
        @session.find(:id, 'test_field', filter_set: :field, with: 'not_monkey')
      }.to raise_error(Capybara::ElementNotFound)
    end
    ...
    it 'should allow importing of filter set into selector' do
      ...
      expect {
        @session.find(:id_with_field_filters, 'test_field', with: 'not_monkey')
      }.to raise_error(Capybara::ElementNotFound)
    end
  end
  ...
  context 'with :exact option' do
    ...
    it 'matches loosely when false' do
      expect(@session.find(:xpath, XPath.descendant(:input)[XPath.attr(:id).is('test_field')],
                           exact: false).value).to eq('monkey')
    ...
    context 'when set to `smart`' do
      context 'and `exact` set to `false`' do
        ...
        it 'finds a single exact match when there also are inexact matches' do
          result = @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('almost_singular')], match: :smart,
                                                                                                     exact: false)
        ...
        it 'finds a single inexact match' do
          result = @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('almost_singular but')], match: :smart,
                                                                                                         exact: false)
        ...
        it 'raises an error if there is no match' do
          expect do
            @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('does-not-exist')], match: :smart,
                                                                                             exact: false)
          end.to raise_error(Capybara::ElementNotFound)
        end
      end
      ...
      context 'with `exact` set to `true`' do
        ...
        it 'raises an error when there is a single inexact matches' do
          expect do
            @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('almost_singular but')], match: :smart,
                                                                                                  exact: true)
          end.to raise_error(Capybara::ElementNotFound)
        end
    ...
    context 'when set to `prefer_exact`' do
      context 'and `exact` set to `false`' do
        it 'picks the first one when there are multiple exact matches' do
          result = @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('multiple')], match: :prefer_exact,
                                                                                              exact: false)
        ...
        it 'finds a single exact match when there also are inexact matches' do
          result = @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('almost_singular')],
                                 match: :prefer_exact, exact: false)
          expect(result.text).to eq('almost singular')
        end
        ...
        it 'raises an error if there is no match' do
          expect do
            @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('does-not-exist')], match: :prefer_exact,
                                                                                             exact: false)
          end.to raise_error(Capybara::ElementNotFound)
        end
      end
      ...
      context 'with `exact` set to `true`' do
        ...
        it 'finds a single exact match when there also are inexact matches' do
          result = @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('almost_singular')],
                                 match: :prefer_exact, exact: true)
          expect(result.text).to eq('almost singular')
        end

        it 'raises an error if there are multiple inexact matches' do
          expect do
            @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('almost_singul')], match: :prefer_exact,
                                                                                            exact: true)
          end.to raise_error(Capybara::ElementNotFound)
        end
        ...
        it 'raises an error if there is a single inexact match' do
          expect do
            @session.find(:xpath, XPath.descendant[XPath.attr(:class).is('almost_singular but')], match: :prefer_exact,
                                                                                                  exact: true)
          end.to raise_error(Capybara::ElementNotFound)
        end
