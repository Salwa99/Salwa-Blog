      ...
      def compute_if_absent(key)
        ...
        while true
          if !(node = current_table.volatile_get(i = current_table.hash_to_index(hash)))
            ...
            succeeded, value = attempt_internal_compute_if_absent(key, hash, current_table, i, node, node_hash) {
              yield
            }
      ...
      def internal_replace(key, expected_old_value = NULL, &block)
        ...
        while current_table
          if !(node = current_table.volatile_get(i = current_table.hash_to_index(hash)))
            ...
            succeeded, old_value = attempt_internal_replace(key, expected_old_value, hash, current_table, i, node,
                                                            node_hash, &block)
      ...
      def rebuild(table)
        ...
        while true
          if !(node = table.volatile_get(i))
            ...
            redo unless (bin >= 0 ? table.cas(i, nil,
                                              forwarder) : lock_and_clean_up_reverse_forwarders(table, old_table_size,
                                                                                                new_table, i, forwarder))
