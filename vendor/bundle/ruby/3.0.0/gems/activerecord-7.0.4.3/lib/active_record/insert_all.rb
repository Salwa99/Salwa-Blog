...
module ActiveRecord
  class InsertAll # :nodoc:
    ...
    def initialize(model, inserts, on_duplicate:, update_only: nil, returning: nil, unique_by: nil,
    ...
    def find_unique_index_for(unique_by)
      ...
      if index = unique_indexes.find { |i| match.include?(i.name) || i.columns == match }
        ...
        unique_by.nil? ? nil : ActiveRecord::ConnectionAdapters::IndexDefinition.new(model.table_name,
                                                                                     "#{model.table_name}_primary_key", true, match)
      else
    ...
    class Builder # :nodoc:
      ...
      delegate :skip_duplicates?, :update_duplicates?, :keys, :keys_including_timestamps, :record_timestamps?,
               to: :insert_all
